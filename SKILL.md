...
- By default, do not include a JCR/impact-factor table in the Word report. Cached journal metrics are available in `references/journal_metrics.json` if the user explicitly asks for them.

## Required Workflow

1. Read `references/workflow.md` for the search, screening, categorization, and report rules.
2. Read `references/journal_metrics.json` only when constructing PubMed journal queries or when the user asks for journal metrics.
...
- Created: 2026-06-09.
- Use these cached values to avoid web lookup during routine weekly reports.
